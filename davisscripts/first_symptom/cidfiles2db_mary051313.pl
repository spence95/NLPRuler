...
if ($count==0) {
        ...
        print "Table " . $tables[0] . " exists in database '$db' but not the other required db.  Either both 'concepts' and 'concepts_data' tables are required (or neither).  If neither is present, the system will create the
m.";
        exit;
}
...
for my $f (@files) {
    ...
    while ($a = <IN>) {
        ...
        db_execute($dbh, "INSERT INTO concepts_data (ruid,cui,sentnum,wordnum,score,attr,orig_text,section_string)
                   VALUES ($ruid,$cui,$sentnum,$wordnum,$score,$attr,$orig_phrase_text,$section_string)");
